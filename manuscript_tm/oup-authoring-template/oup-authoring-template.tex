...
To ensure stability, the Gradient Penalty weight was fixed at $\lambda_{GP}=10$ \citep{10.5555/3295222.3295327}, and we employed a warmup period for the first 10 epochs where $\lambda_{\text{adv}}=0$ and the adversary's weights were frozen. This allows for valid autoencoder reconstruction before adversarial alignment. We utilized class-balanced sampling with replacement across all experiments; this ensured equal representation of all source batches within each mini-batch, which both prevented mode collapse in smaller experimental batches and guaranteed well-defined interpolations for the Gradient Penalty. Models were trained and evaluated using a nested cross-validation scheme (5 outer folds, 3 inner folds), where inner loops ran for 40 epochs and outer loops for 80 epochs. Hyperparameters were systematically selected to maximize the composite integration score, $(\text{iLISI} - 10*\text{cLISI})$, on held-out data, where the cLISI penalty is scaled by a factor of 10 to balance the magnitude of the respective metrics.
