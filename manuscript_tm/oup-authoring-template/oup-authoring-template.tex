...
\subsection{Optimal transport and Wasserstein barycenters for data integration}
A recurring difficulty for any reference-based Wasserstein alignment is the choice of the anchor: aligning every batch to one designated batch makes the result only as good as that reference, and a sparse or unrepresentative anchor propagates its gaps to the entire integration \citep{ijms24065502}. The classical remedy in optimal transport is the \textit{Wasserstein barycenter}---the Fr\'echet mean of a set of distributions under the Wasserstein metric \citep{doi:10.1137/100805741}---which replaces a designated reference with a virtual center that is by construction the geometric median of all batches. This idea has been developed most fully in multi-source domain adaptation, where the source distributions are aggregated into their Wasserstein barycenter to form an intermediate domain that is then transported to the target \citep{Montesuma2021-wbt}, and it underlies related dictionary-learning and Monge-alignment schemes for aligning several domains to a shared support. Within single-cell analysis, optimal transport has been applied both to trajectory and integration problems \citep{Ma2022-otsc}, and the barycenter itself has been used for integration at the level of clustering aggregation: MAW \citep{Lin2023-maw} represents each dataset as a Gaussian mixture and integrates the datasets through a minimized aggregated Wasserstein approximation to the barycenter of those mixtures. That construction operates on fitted mixture models as a post-hoc aggregation step rather than as a training objective, and it is not adversarial. To our knowledge, the barycenter has not previously been used as a \textit{learnable, end-to-end alignment target inside an adversarial critic}: rather than aggregating pre-computed models, we let the generator optimize a set of anchor points that migrate toward the Fr\'echet mean of the batch distributions in latent space, so that the same Wasserstein-critic gradients that drive reference-based alignment instead drive alignment to a shared virtual center. This directly targets the two failure modes we document for the reference critic---its dependence on a well-chosen, topologically dense reference batch and its tendency to over-correct---and situates the barycenter formulation as the natural continuation of, rather than a departure from, the multi-headed critic we benchmark here.
...
A unique architectural feature of our multi-headed critic is its alignment to a specific reference manifold. While this avoids the $O(\lvert\mathcal{V}\rvert^2)$ complexity of all-versus-all matching, it introduces a dependency on the quality of the chosen reference batch. Except where the reference is swept explicitly (below), we therefore select it by \textit{maximum cell-type Shannon entropy}, $H = -\sum_c p_c \log p_c$ over the cell-type proportions within a batch, with ties broken by cell count. This choice follows directly from the sensitivity analysis reported here: performance depends on how completely the reference represents the biology, not on how many cells it contains, so entropy is the property a reference should maximise. Selecting the \textit{largest} batch instead optimises for cell count and can anchor alignment on a batch dominated by one or two populations. On our benchmarks the two criteria disagree on half the datasets --- for Pancreas, entropy selects a batch covering all 14 cell types ($H=1.851$) over the largest batch ($H=1.757$) --- and the rule is pre-registered rather than tuned. To quantify this, we exhaustively trained models using every possible batch as the reference target and analyzed the variance in performance (Figure \ref{fig:reference_sensitivity}).
...
\textbf{From Reference Matching to Wasserstein Barycenters.} The Wasserstein critic is further limited by the sparsity inherent to the reference matching induced by our multi-headed formulation, where a single reference batch acts as the central hub for $\lvert\mathcal{V}\rvert-1$ target batches. While this topology avoids the quadratic complexity of all-vs-all matching, it is topologically fragile: if the reference is incomplete, the entire alignment suffers. The principled solution to this limitation is the \textit{Wasserstein barycenter} \citep{doi:10.1137/100805741}, which replaces the fixed reference with a virtual center---the geometric median of all batch distributions---as discussed in relation to prior optimal-transport work above. In contrast to the barycenter constructions used elsewhere for integration, which aggregate pre-fitted mixture models as a post-hoc step \citep{Lin2023-maw} or align domains to a barycenter outside any adversarial objective \citep{Montesuma2021-wbt}, our formulation makes the center a set of learnable anchors that the generator migrates toward the Fr\'echet mean during training, so the same critic gradients that drive reference-based alignment instead drive alignment to a shared virtual center. This aligns all batches to a common manifold spanning the union of biological states without designating any single batch as the hub, and is intended to combine the robust gradients of the Wasserstein critic with the global robustness of the discriminator. Systematically evaluating this barycenter formulation against the reference critic and the discriminator across the full benchmark is the direct continuation of the present work.
...
\section{Methods}
Our method separates cleanly into two parts: a general adversarial deconfounding framework that operates entirely on the latent code $z$, and an autoencoder backbone that produces $z$ from gene expression. Because the adversarial head never sees gene space---it acts only on $z$---the framework is backbone-agnostic: any encoder can be substituted without changing the objective. We exploit this separation to isolate the adversarial objective (holding the backbone fixed, \S\ref{sec:framework}) and, separately, to test whether our findings depend on the specific backbone (varying it, \S\ref{sec:backbones}). We first define the framework, then the backbones, then implementation and evaluation.

\subsection{Adversarial deconfounding framework}\label{sec:framework}
The task is to learn a latent representation $z$ in which cells are organized by biology but not by batch. We cast this as a two-player adversarial game over batch identity, played on the latent code. Let the encoder map each cell to $z$, and let the batches induce per-batch latent distributions $P_1,\dots,P_{\lvert\mathcal{V}\rvert}$. An adversarial \textit{head}, operating only on $z$, quantifies how distinguishable these batch distributions are through a measure $\mathcal{D}(P_1,\dots,P_{\lvert\mathcal{V}\rvert})$. The head is trained to \textit{maximize} $\mathcal{D}$ (make batches distinguishable), while the encoder acts as a generator trained to \textit{minimize} it (make batches indistinguishable---i.e.\ mix them)---subject to the backbone losses that keep $z$ faithful to biology:
\begin{equation}
    \min_{\text{encoder}}\ \max_{\text{head}}\quad \mathcal{L}_{\text{backbone}}(\text{encoder}) \;+\; \lambda_{\text{adv}}\,\mathcal{D}\big(P_1,\dots,P_{\lvert\mathcal{V}\rvert}\big),
    \label{eq:framework}
\end{equation}
where $\mathcal{L}_{\text{backbone}}$ is the reconstruction and regularization loss of the chosen autoencoder (\S\ref{sec:backbones}) and $\lambda_{\text{adv}}$ weights the adversarial term. Minimizing $\mathcal{D}$ with respect to the encoder \textit{is} batch mixing; the heads below differ \textit{only} in the choice of $\mathcal{D}$ and how it is estimated, which is precisely what makes this a controlled comparison of adversarial objectives. A schematic of the framework and the interchangeable head is shown in Figure \ref{fig1}.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{arch_redrawn.png}
    \caption{\textbf{Experimental architecture.} A variational autoencoder encodes expression $X$ into a latent code $z$ and reconstructs $\hat{X}$; the decoder is optionally conditioned on the batch label $v$. A single adversarial head attached to $z$ removes batch information --- exactly one objective per model, either a discriminator (a $\lvert\mathcal{V}\rvert$-way classifier trained by multi-class cross-entropy, approximating the Jensen--Shannon divergence) or a Wasserstein critic (a multi-headed network constrained 1-Lipschitz by a gradient penalty; we evaluate several critic formulations that differ in what each batch is aligned to). We compare the two heads across a panel of VAE backbones (Table~\ref{tab:backbones}).}
    \label{fig1}
\end{figure}

\subsubsection{Discriminator baseline (Jensen--Shannon divergence)}
To serve as a baseline for comparison, we implement a standard adversarial deconfounding approach using a discriminator network $h: \mathcal{Z}\rightarrow\mathbb{R}^{\lvert\mathcal{V}\rvert}$ --- a single $\lvert\mathcal{V}\rvert$-way classifier that outputs one logit per batch --- which attempts to predict the confounding variable $v \in \mathcal{V}$ (e.g., batch ID) from the latent embedding code $z \in \mathcal{Z}$. The discriminator is trained to minimize the multi-class cross-entropy prediction error, effectively approximating the Jensen-Shannon divergence between confounding distributions from the data source, $\mathcal{D}$:
\begin{equation*}
    \min_h \mathbb{E}_{\tilde{x} \sim \mathcal{D}, z \sim q_\phi(z|\tilde{x})} \left[ \mathcal{L}_{\text{CE}}(h(z), v) \right]
\end{equation*}
...
\subsubsection{Reference-based Wasserstein critic}\label{sec:refcritic}
...
\subsubsection{Pooled and barycenter critic formulations}\label{sec:altcritic}
% [FILL from E9 results: pooled (global-pool target, V-way joint) and barycenter
%  (learnable Frechet-mean anchors) formulations; jobs E9_immune/pancreas/immune_hum_mou
%  + E9scale_pancreas running. Describe D for each and the learnable-anchor optimization.]

\subsubsection{Per-head encoder objectives}
...
\subsection{Autoencoder backbones}\label{sec:backbones}
The adversarial framework above attaches only to the encoder output $z$ and is therefore agnostic to the choice of autoencoder. To test whether the critic-versus-discriminator trade-off depends on the generative model, we evaluate the adversarial heads on a panel of established variational autoencoder (VAE) backbones spanning the decoder likelihoods in common single-cell use, applying the identical adversarial head, training loop, and evaluation to each. We use each backbone in its originally published form, trained with only its own reconstruction likelihood and the KL regularizer \citep{Kingma2014}:
\begin{equation*}
    \mathcal{L}_{\text{VAE}} = \mathcal{L}_{\text{recon}} + \beta_{\text{KL}}\,\mathrm{KL}\!\left(q_\phi(z\mid\tilde{x})\,\|\,\mathcal{N}(0,I)\right)
\end{equation*}
with $\beta_{\text{KL}}=0.005$. All backbones share a common two-layer encoder $q_\phi(z\mid\tilde{x})$ ($\lvert\tilde{x}\rvert\!\rightarrow\!1024\!\rightarrow\!512$, then parallel linear maps to the mean and variance of $Z\sim\mathcal{N}(\mu,\sigma^2)$, $\lvert z\rvert=256$) and, unless stated otherwise, a two-layer decoder trunk ($\cdot\!\rightarrow\!512\!\rightarrow\!1024$) feeding per-gene parameter heads. Isolating the decoder likelihood as the sole source of variation across the panel ensures that any backbone-dependence of the trade-off is attributable to the generative assumption rather than to confounding modeling choices.

A second, orthogonal design axis is decoder batch conditioning. Supplying the batch label to the decoder is itself a form of batch correction --- a conditioned decoder can absorb batch effects that the encoder would otherwise have to remove --- so it is entangled with the adversarial head's job. We therefore treat conditioning as an explicit variable and evaluate every count backbone in both a conditioned and an unconditioned form: a conditioned decoder receives the batch one-hot $v$ concatenated to $z$, while an unconditioned decoder receives $z$ alone. Crossing decoder likelihood with conditioning yields the backbone panel summarized in Table~\ref{tab:backbones}; the paragraphs below describe each likelihood, ordered from simplest to most expressive.

\begin{table}[t]
\caption{Autoencoder backbone panel: decoder likelihood $\times$ batch conditioning.
Each configuration is trained with both the discriminator and the Wasserstein-critic head.}
\label{tab:backbones}
\centering
\begin{tabular}{llcc}
\toprule
Backbone & Decoder likelihood & Conditioned & Unconditioned \\
\midrule
Gaussian & Gaussian (MSE)        & --          & \checkmark \\
Poisson  & Poisson               & --          & \checkmark \\
NB       & Negative binomial     & \checkmark  & \checkmark \\
ZINB     & Zero-inflated NB      & \checkmark  & \checkmark \\
LDVAE    & NB, linear decoder    & \checkmark  & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Gaussian VAE}
The simplest control \citep{Kingma2014}: a Gaussian likelihood on the log-normalized input, optimized as mean-squared reconstruction error, $\mathcal{L}_{\text{recon}}=\lVert \hat{x}-\tilde{x}\rVert_2^2$. As it has no count model and no natural batch-covariate pathway, it is evaluated unconditioned only.

\subsubsection{Poisson VAE}
The simplest count likelihood \citep{Townes2019-multinomial}. The decoder emits a per-gene rate $\lambda_g=\exp(\cdot)$, and reconstruction is the Poisson negative log-likelihood $\mathcal{L}_{\text{recon}}=\frac{1}{G}\sum_g \big(\lambda_g - x_g\log\lambda_g\big)$ (dropping the data-independent $\log x_g!$ term).

\subsubsection{Negative-binomial (NB) VAE}
The standard single-cell count model \citep{Lopez2018-bl}. The decoder emits a per-gene mean $\mu_g=\exp(\cdot)$ and an \emph{inverse-dispersion} $\theta_g=\exp(\cdot)$, and reconstruction is the NB negative log-likelihood, computed as the mean over genes $G$ for gradient scale-invariance across dataset sizes:
\begin{equation}
    \mathcal{L}_{\text{recon}} = -\frac{1}{G}\sum_{g=1}^{G} \log P_{\text{NB}}\!\left(x_g \mid \mu_g, \theta_g\right)
    \label{eq:nb}
\end{equation}
We confirmed this likelihood is numerically identical to the reference \texttt{scvi-tools} implementation. It is evaluated both conditioned and unconditioned.

\subsubsection{Zero-inflated NB (ZINB) VAE}
Extends the NB decoder with a per-gene dropout-logit head $\pi_g$, giving a zero-inflated NB likelihood \citep{Risso2018-zinbwave} that models excess zeros beyond the NB expectation. Evaluated conditioned and unconditioned.

\subsubsection{Linearly-decoded VAE (LDVAE)}
The interpretable scVI variant \citep{Svensson2020-ldvae}: the same NB likelihood as above, but with a \emph{single linear} decoder mapping the (optionally conditioned) latent directly to the per-gene parameters, trading decoder capacity for interpretable gene loadings. Evaluated conditioned and unconditioned.

\subsection{Implementation, Training, and Model Selection}
To evaluate performance across varying degrees of topological complexity and batch overlap, we utilized three standard benchmark datasets \citep{Luecken2022-fb}: \textbf{Pancreas} (9 batches), \textbf{Lung} (2 batches), and \textbf{Immune} (4 batches). All models were implemented in PyTorch and trained on a single NVIDIA RTX 4090 GPU using the Adam optimizer (learning rate = 0.001, $\beta_1=0.5, \beta_2=0.9$); the $\beta_1$ momentum parameter was intentionally reduced to stabilize adversarial dynamics. Models were trained with a batch size of 1024 for up to 500 epochs, with early stopping on a held-out cell-type probe (evaluated every 10 epochs, patience 5, best weights restored). This replaces a fixed epoch budget, which we found acts as an undisclosed regulariser: in a convergence experiment at 150, 300 and 450 epochs (independent runs, three datasets, both adversaries), batch mixing was flat in the training budget (iLISI 0.605, 0.606, 0.619) while cell-type conservation degraded substantially (ARI 0.058, 0.015, 0.018; held-out cell-type probe lift 0.168, 0.132, 0.097). The conservation-optimal budget differed by dataset and by adversary, so no single fixed value is neutral between the two methods. Due to the multiple local updates required to enforce the Lipschitz constraint (10 critic updates per generator step), the Wasserstein-based models required approximately ~24\% more training time per epoch than the baseline discriminator (see detailed breakdown in Supplementary Figure S1 and Supplementary Table S1).

To ensure stability, the Gradient Penalty weight was fixed at $\lambda_{GP}=10$ \citep{10.5555/3295222.3295327}, and we employed a warmup period for the first 10 epochs where $\lambda_{\text{adv}}=0$ and the adversary's weights were frozen. This allows for valid autoencoder reconstruction before adversarial alignment. We utilized class-balanced sampling with replacement across all experiments; this ensured equal representation of all source batches within each mini-batch, which both prevented mode collapse in smaller experimental batches and guaranteed well-defined interpolations for the Gradient Penalty. Models were trained and evaluated using a nested cross-validation scheme (5 outer folds, 3 inner folds). Within each training fold, $\lambda_{\text{adv}}$ was selected over the grid $\{0, 0.01, 0.02, 0.05, 0.1, 0.2, 0.35, 0.5, 0.75, 1.0\}$ and the selected value applied to the held-out fold, so that no held-out data informed hyperparameter choice. Selection maximised the scIB overall score, $0.4 \cdot \text{(batch correction)} + 0.6 \cdot \text{(bio conservation)}$, with each category min--max scaled within dataset and averaged before combination \citep{Luecken2022-fb}. We replaced our earlier composite, $(\text{iLISI} - 10 \cdot \text{cLISI})$, because the $\times 10$ weighting made the criterion effectively a single-metric objective: across the $\lambda_{\text{adv}}$ sweep iLISI varied by 0.021 while cLISI varied by 0.036, so the penalty term dominated by a factor of 17.7. The scIB weighting is insensitive to its exact value here --- the selected $\lambda_{\text{adv}}$ was unchanged for weights in $[0.3, 0.6]$.
...
\textbf{Global Metrics (Secondary).} To assess macro-scale alignment, we employ standard metrics calculated using the \texttt{scib} Python framework \citep{Luecken2022-fb}. To ensure consistent interpretability, we utilize the scaled versions of these metrics, where \textit{higher values always indicate better performance}: Metrics were computed with \texttt{scib} 1.1.7. Because ARI/NMI and the isolated-label $F_1$ score each require a Leiden resolution sweep, and \texttt{scib} performs an independent sweep per metric (and one per isolated label), we compute each resolution once and let every metric select its own optimum from the shared clusterings. This is numerically identical to the per-metric sweeps ($|\Delta\text{ARI}| = 5 \times 10^{-18}$, $|\Delta\text{NMI}| = 0$, $|\Delta F_1| = 0$ on the pancreas benchmark) at 1.9$\times$ lower cost. Clustering uses the \texttt{igraph} Leiden implementation with \texttt{n\_iterations} $=2$, matching the iteration count \texttt{leidenalg} applies by default. Evaluated twice on the same fixed embedding, the two implementations agree to $|\Delta\text{ARI}| = 1.2 \times 10^{-4}$ and leave every clustering-free metric bit-identical, reducing suite cost 1.55$\times$; the full equivalence assessment, including the largest residual, is given in Supplementary Note 4.
\begin{itemize}
