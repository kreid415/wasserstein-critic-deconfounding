"""VAE backbone and base likelihoods.

PROVENANCE: Derived from scCRAFT (He et al., 2025; github.com/ch2343/scCRAFT),
licensed under its original terms (see scCRAFT/NOTICE). The Encoder/Decoder/VAE
and log_nb_positive are upstream architecture; local modifications by K. Reid add
a `warmup` flag to the encoder forward, a separate `x_raw` counts path to VAE.forward
for the NB reconstruction, and the batch-effect decoder pathway. CrossEntropy and
reparameterize_gaussian are upstream. The Wasserstein critic, the adversarial-head
dispatch, alternative backbones, the training engine, evaluation metrics, data prep,
and the CV/Pareto harness are NOT here — they are the authored contribution under
the sibling ``wcd/`` package.
"""
